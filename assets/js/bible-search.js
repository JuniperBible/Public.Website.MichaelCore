...
/**
 * Search request counter to prevent race conditions
 * Incremented for each new search, used to ignore stale results
 * @type {number}
 */
 ...
 * Uses searchId counter to ensure only the most recent search displays results.
 * Stale searches are abandoned even if they complete.
 *
 * @async
...
async function performSearch() {
  // Assign unique ID to this search request
  ...
  try {
    ...
    for (const book of bibleData.books) {
      for (let ch = 1; ch <= book.chapters; ch++) {
        // Check if this search is stale (a newer search has started)
        if (searchId !== currentSearchId) return;

        // Check if search was canceled via AbortController
    ...
    // Check one final time before rendering results
    if (searchId !== currentSearchId) return;
...
export { performSearch, initFromUrl };

// Backwards compatibility with window.Michael namespace
window.Michael = window.Michael || {};
window.Michael.BibleSearch = {
  performSearch,
  init: initFromUrl
};
