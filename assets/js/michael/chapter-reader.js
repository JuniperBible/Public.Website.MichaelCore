...
/**
 * Fetch comparison verses via the BibleAPI module
 * @param {string} signal - AbortSignal to cancel the request
 * @returns {Promise<Array>} Array of verse objects with number and html properties
 */
...
/**
 * Fetch comparison verses by retrieving and parsing the chapter HTML page
 * @param {AbortSignal} signal - AbortSignal to cancel the request
 * @returns {Promise<Array|null>} Array of verse objects, or null if aborted
 */
async function fetchViaHTML(signal) {
...
async function loadAndRenderSSS() {
  ...
  try {
    ...
    if (window.Michael && window.Michael.BibleAPI) {
      const result = await fetchViaAPI(signal);
      if (result === null) return;
      rightVerses = result;
    } else {
      // Fallback: fetch HTML directly and parse verses
      const result = await fetchViaHTML(signal);
      if (result === null) return;
      rightVerses = result;
    }
