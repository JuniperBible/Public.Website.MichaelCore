...
function init(config) {
  ...
  setupSSSButtonListeners(config);
  setupSSSDropdownListeners(config);
}

/** Set up button and toggle event listeners for SSS mode */
function setupSSSButtonListeners(config) {
...
/** Set up select/dropdown event listeners for SSS mode */
function setupSSSDropdownListeners(config) { // eslint-disable-line no-unused-vars
...
/** Reset SSS state to empty when entering a new day */
function resetSSSStateForNewDay() {
...
/** Apply default SSS selection values if not already set */
...
/** Handle a fetch error in SSS comparison load */
function handleSSSFetchError(err) {
...
/**
 * Build the translation label header element for an SSS pane
 * @param {Object} bible - Bible metadata for this pane
 * @param {Object} compareBible - Bible metadata for the opposing pane
