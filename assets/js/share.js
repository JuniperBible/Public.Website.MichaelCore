...
/**
 * Build a verse share button element for a given verse number
 * @param {string} num - Verse number
 * @param {number} iconSize - Icon size in pixels
 * @returns {HTMLButtonElement} Configured share button
 */
function buildVerseShareButton(num, iconSize) {
  ...
  btn.appendChild(createSvgIcon(SVG_PATHS.share, iconSize));
...
/**
 * Add share buttons using modern verse span format
 * @param {NodeList} verseSpans - Span elements with data-verse attributes
 * @returns {void}
 */
function addModernVerseButtons(verseSpans) {
  verseSpans.forEach(span => {
    const num = span.dataset.verse;
    const sup = span.querySelector('sup');
    if (!sup) return;

    const btn = buildVerseShareButton(num, 12);
    sup.parentNode.insertBefore(btn, sup.nextSibling);
  });
}

/**
 * Add share buttons using legacy strong-element verse format
 * @param {NodeList} verses - Strong elements containing verse numbers
 * @returns {void}
 */
function addLegacyVerseButtons(verses) {
...
function addVerseShareButtons(container) {
  ...
  if (verseSpans.length > 0) {
    addModernVerseButtons(verseSpans);
    return;
  }

  // Legacy fallback: <strong>N</strong> format
  addLegacyVerseButtons(container.querySelectorAll('strong'));
}
...
/**
 * Extract verse text from a modern span.verse element
 * @param {HTMLElement} verseSpan - The span element with class "verse"
 * @param {string} title - Chapter title for reference prefix
 * @param {string} verseNum - Verse number
 * @returns {string} Formatted verse text with reference
 */
function getVerseTextModern(verseSpan, title, verseNum) {
  let text = '';
...
/**
 * Extract verse text using the legacy strong-element format
 * @param {HTMLElement} bibleText - The .bible-text container element
 * @param {string} title - Chapter title for reference prefix
 * @param {string} verseNum - Verse number
 * @returns {string} Formatted verse text with reference, or empty string if not found
 */
function getVerseTextLegacy(bibleText, title, verseNum) {
...
function getVerseText(verseNum) {
  ...
  if (verseSpan) {
    return getVerseTextModern(verseSpan, title, verseNum);
  }

  // Legacy fallback: <strong>N</strong> format
  return getVerseTextLegacy(bibleText, title, verseNum);
}
 ...
 * @listens load
 */
window.addEventListener('load', handleVerseDeepLink);

// ============================================================================
// PUBLIC API
// ============================================================================

window.Michael.share = {
  init,
  shareChapter
};
